...
public class CreateJSON {
	...
	public static JsonObject createjson(PubsubMessage message, String subscriptionId, int log_type) {
		...
		case 1:
			...
			break;
		case 2:
			data_json.addProperty("Log type", "status");
			data_json.addProperty("App Status", "Starting");
	...
	public static Instant Convert_ts(String time_st) {
		...
		try {
			...
			LocalDateTime localDateTime = LocalDateTime.from(formatter.parse(time_st));
			Timestamp payload_ts = Timestamp.valueOf(localDateTime);
			return payload_ts.toLocalDateTime().toInstant(ZoneOffset.UTC);
		} catch (Exception e) {
			return Instant.now();
		}
